...
class EventListener implements Listener
{
    ...
    public function onChat(PlayerChatEvent $e)
    {
        ...
        if (!$e->isCancelled())
        {
            ...
            $limitCharNumber = $this->getConfigValue("antispam.limitCharNumber", 100);
            
            if ($antiSpamEnabled)
            {
                if(strlen($msg) > $limitCharNumber)
                {
                    $msg = substr($msg, 0, $limitCharNumber);
                }
            ...
            {
                $msg = $this->processBlockedWords($msg);
            }
        
            if ($ipLeakBlockEnabled && $this->hasIpAddress($msg))
            {
                $msg = $this->blockIpLeak($msg);
            }
